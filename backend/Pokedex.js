function getRandomPokemon(callback) {
    ...
    getPokemonByResourceURI("/api/v1/pokemon/" + rand + "/", callback);
}

function getPokemonByName(pokemonName, callback) {
    allPokemon = pokedex.pokemon;
    for (var i = 0; i < allPokemon.length; i++) {
        if (allPokemon[i].name === pokemonName) {
            getPokemonByResourceURI(allPokemon[i].resource_uri, callback);
        }
    }
}

function getPokemonByResourceURI(resourceURI, callback) {
